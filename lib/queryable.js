...
Queryable.prototype.findOnePromise = util.promisify(Queryable.prototype.findOne);
...
Queryable.prototype.countPromise = util.promisify(Queryable.prototype.count);


/**
 * Check if rows exist and calls back with any error and the result. There are two ways to use this method:
 *
 * 1. find() style: db.mytable.exists({field: value}, callback);
 * 2. where() style: db.mytable.exists("field=$1", [value], callback);
 */
Queryable.prototype.exists = function() {
  var args;
  var where;
  ...
  var sql = "select exists(select 1 from " + this.delimitedFullName + where.where + ") as ok";
  
  console.log(where.params);
  
  this.db.query(sql, where.params || args.params, {single : true}, function(err, res) {
    if (err) args.next(err, null);
    else args.next(null, res.ok);
  });
};
Queryable.prototype.existsSync = DA(Queryable.prototype.exists);
Queryable.prototype.existsPromise = util.promisify(Queryable.prototype.exists);
...
Queryable.prototype.wherePromise = util.promisify(Queryable.prototype.where);

Queryable.prototype.find = function(){
  var args = ArgTypes.findArgs(arguments);
  var pk = (this.hasOwnProperty("pk") ? util.format('"%s"', this.pk) : "1");
  if (_.isArray(this.pk)) {
	  var kk = [];
	  _.forEach(this.pk, function(k) {
		  kk.push(util.format('"%s"', k));
	  });
	  pk = kk.join(" , ");
  }
  //set default options
  //if our inheriting object defines a primary key use that as the default order
  args.options.order = args.options.order || pk;
...
Queryable.prototype.findPromise = util.promisify(Queryable.prototype.find);
...
Queryable.prototype.searchPromise = util.promisify(Queryable.prototype.search);
